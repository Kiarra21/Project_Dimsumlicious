...
Route::prefix('admin/products')->middleware(['auth', 'role:admin,staff'])->name('products.')->group(function () {
    ...
    Route::delete('/{id}', [ProductController::class, 'destroy'])->middleware('role:admin')->name('destroy');
});

// Category Management (Admin & Staff)
Route::prefix('categories')->middleware(['auth', 'role:admin,staff'])->name('categories.')->group(function () {
    Route::get('/', [CategoryController::class, 'index'])->name('index');
    Route::post('/', [CategoryController::class, 'store'])->name('store');
    Route::put('/{id}', [CategoryController::class, 'update'])->name('update');
    Route::delete('/{id}', [CategoryController::class, 'destroy'])->middleware('role:admin')->name('destroy');
});
...
Route::prefix('orders')->middleware(['auth', 'role:admin,staff'])->name('orders.')->group(function () {
    ...
    Route::delete('/{id}', [App\Http\Controllers\OrderController::class, 'destroy'])->middleware('role:admin')->name('destroy');
});
...
// Payment Management (Admin & Staff)
Route::prefix('payments')->middleware(['auth', 'role:admin,staff'])->name('payments.')->group(function () {
    Route::get('/', [App\Http\Controllers\PaymentController::class, 'index'])->middleware('permission:payment.view')->name('index');
    Route::get('/{id}', [App\Http\Controllers\PaymentController::class, 'show'])->middleware('permission:payment.view')->name('show');
    Route::post('/{id}/verify', [App\Http\Controllers\PaymentController::class, 'verify'])->middleware('permission:payment.verify')->name('verify');
    Route::post('/{id}/reject', [App\Http\Controllers\PaymentController::class, 'reject'])->middleware('permission:payment.verify')->name('reject');
});

// Staff Management (Admin Only)
Route::prefix('staff')->middleware(['auth', 'role:admin'])->name('staff.')->group(function () {
...
Route::prefix('promos')->middleware(['auth', 'role:admin,staff'])->name('promos.')->group(function () {
    Route::get('/', [PromoController::class, 'index'])->name('index');
    Route::post('/', [PromoController::class, 'store'])->name('store');
    Route::put('/{id}', [PromoController::class, 'update'])->name('update');
    Route::delete('/{id}', [PromoController::class, 'destroy'])->middleware('role:admin')->name('destroy'); // Only admin can delete
});

// Company Profile (Admin Only)
Route::prefix('company-profile')->middleware(['auth', 'role:admin'])->name('company-profile.')->group(function () {
    Route::get('/', [CompanyProfileController::class, 'index'])->name('index');
    Route::put('/', [CompanyProfileController::class, 'update'])->name('update');
});

// Profile (Admin & Staff)
Route::prefix('profile')->middleware(['auth', 'role:admin,staff'])->name('profile.')->group(function () {
